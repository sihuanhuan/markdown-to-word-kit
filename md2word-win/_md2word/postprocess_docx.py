...
def set_paragraph_indent(paragraph: ET.Element, *, left: str = "0", first_line: str = "0") -> None:
    p_pr = ensure_child(paragraph, "pPr")
    remove_children(p_pr, "ind")
    add_element(p_pr, "ind", {"left": left, "firstLine": first_line})


def paragraph_num_id_and_level(paragraph: ET.Element) -> tuple[str, int] | None:
    p_pr = child(paragraph, "pPr")
    if p_pr is None:
        return None
    ...
    if num_pr is None:
        return None
    num_id = child(num_pr, "numId")
    if num_id is None:
        return None
    raw_num_id = num_id.get(w_attr("val"))
    if not raw_num_id:
        return None

    ilvl = child(num_pr, "ilvl")
    raw_ilvl = ilvl.get(w_attr("val")) if ilvl is not None else "0"
    try:
        level = int(raw_ilvl or "0")
    except ValueError:
        level = 0
...
def simple_list_kind(num_fmt: str, lvl_text: str) -> str | None:
    if num_fmt == "decimal":
        return "ordered"
    if num_fmt != "bullet":
        return None
    if any(mark in lvl_text for mark in ("☒", "☑", "✓", "✔")):
        return "task_checked"
    if any(mark in lvl_text for mark in ("☐", "□")):
...
def collect_simple_list_kinds(numbering_xml: Path) -> dict[tuple[str, int], str]:
    ...
    for abstract in root.findall("w:abstractNum", NS):
        abstract_id = abstract.get(w_attr("abstractNumId"))
        if not abstract_id:
            continue

        levels: dict[int, str] = {}
        for level in abstract.findall("w:lvl", NS):
            raw_ilvl = level.get(w_attr("ilvl")) or "0"
            try:
                ilvl = int(raw_ilvl)
            except ValueError:
                ilvl = 0

            num_fmt = child(level, "numFmt")
            lvl_text = child(level, "lvlText")
            kind = simple_list_kind(
                num_fmt.get(w_attr("val")) if num_fmt is not None else "",
                lvl_text.get(w_attr("val")) if lvl_text is not None else "",
            )
            if kind is not None:
                levels[ilvl] = kind
    ...
    for num in root.findall("w:num", NS):
        num_id = num.get(w_attr("numId"))
        abstract = child(num, "abstractNumId")
        abstract_id = abstract.get(w_attr("val")) if abstract is not None else None
        if not num_id or not abstract_id:
            continue
        for ilvl, kind in abstract_levels.get(abstract_id, {}).items():
            kinds[(num_id, ilvl)] = kind
    return kinds


def simple_list_prefix(kind: str, counter: int | None = None) -> str:
    if kind == "bullet":
        return "•　"
    if kind == "ordered":
        return f"[{counter or 1}]　"
    if kind == "task_checked":
        return "☑　"
    if kind == "task_unchecked":
        return "☐　"
    return ""


def apply_simple_list_markers(root: ET.Element, list_kinds: dict[tuple[str, int], str]) -> None:
    counters: dict[tuple[str, int], int] = {}

    for paragraph in root.findall(".//w:p", NS):
        num_info = paragraph_num_id_and_level(paragraph)
        if num_info is None:
            continue

        num_id, level = num_info
        kind = list_kinds.get((num_id, level))
        if kind is None:
            continue
        ...
        prefix = simple_list_prefix(kind, counter)
        if prefix and not paragraph_text(paragraph).startswith(prefix):
            insert_text_prefix(paragraph, prefix)

        remove_paragraph_numbering(paragraph)
        set_paragraph_style_id(paragraph, "Normal")
        set_paragraph_indent(paragraph, left=str(420 * (level + 1)), first_line="0")


def patch_document_xml(path: Path, list_kinds: dict[tuple[str, int], str]) -> None:
    tree = ET.parse(path)
    root = tree.getroot()
    ...
    if body is not None:
        apply_simple_list_markers(body, list_kinds)
