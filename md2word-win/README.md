...
如果你拿到的是 `md2word-win-portable.zip` 便携包，里面已经包含 Python、Pandoc、Node.js 和 Mermaid CLI，解压后可以直接使用 `convert.bat`，通常不需要执行本节安装命令。

如果你拿到的是 `md2word-win-minimal.zip` 最小包，里面不包含 `tools` 目录中的下载资源，需要先执行本节安装命令。
